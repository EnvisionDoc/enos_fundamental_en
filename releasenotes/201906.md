...
- **Resource requesting**: you can request the following computing and storage resources on EnOS Console through specifying resource specifications that match your business needs.
  ...
  - Data warehouse
  - File storage HDFS
- **Resource scale-down**: When your business needs stablize but the resource quota exceeds your actual needs, You can scale down the resource to save costs.
...
An new SDK `HivePartitionDroppedSDK` is added. You can use this SDK to delete the meta data of specified Hive partition and the data stored in the partition. For more information, see [Common Library](/docs/offline-data/en/latest/common_library).

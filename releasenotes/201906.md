...
We added support for TSDB insights, data archiving, data quality report, and meta data exploring. 

#### TSDB Insight

Users can use TSDB data visualization tools to query aggregated AI or PI measureing points, and DI measuring points. The results can be presented in line chart or table. For more information, see [Data Insights Overview](/docs/data-asset/en/latest/howto/storage/data_insights).

.. image:: ../media/data_insights.gif

#### Data Quality Report

Users can query data quality report in terms of completeness, accuracy, and timeliness. Multiple presenting methods are provided under one of the three dimensions, including single device per measuring point per day, multiple devices per measureing point per day, single device per measureing point. For more information, see [Data Quality](/docs/data-asset/en/latest/learn/data_quality).

.. image:: ../media/data_quality.gif

#### Metadata Explorer

Users can search for metadata of measuring points, including basic information, inheritance, storage policy, and instance list. For more information, see [Metadata Explorer](/docs/data-asset/en/latest/learn/metadata_explorer).

.. image:: ../media/data_asset_explorer.gif

#### Data Archiving

We added support for archiving data from real-time data channel. Data of selected model are transmitted and stream analyzed before being stored into user-specified storage systems. For more information, see [Data Archiving](/docs/data-asset/en/latest/howto/archive/archive_policy).


### Data Analytics

We support syncing Azure BLOB storage data into EnOS HDFS. New system variables and data SDK are also added. 

#### Data Integration

Users can now sync Azure BLOB Storage data into EnOS HDFS. For more information, see [Synchronizing Files from External Databases to HDFS](/docs/offline-data/en/latest/data_integration/creating_scratch_blob2hdfs).
...
An new SDK `HivePartitionDroppedSDK` is added to periodically delete Hive data of specified format. Users can delete both the meta data and stored data of specified Hive partition. For more information, see [Common Library](/docs/offline-data/en/latest/common_library).
...
Initial release. SHC provides highly available container management service. It helps project managers create and manage application development, deploy cluster and container, and helps developers quickly build and deploy applications. SHC simplifies buidling clusters that manages containers and is able to create ideal runtime environment for containers in the cloud.

#### Container Service

- Deploys environment and distribute resources for applications and services.
- Supports Kubernetes StatefulSet in automatic container deployment. StatefulSet supports environment variables, thus enabling JVM self-adaptation winthin container. 
- Creates service configuration so that applications can expose web services to outside the cluster. Routing is also configured.
- Uses Route to expose pod ip and cluster ip so that k8s pods can be accessed from outside the cluster.
- Uses ConfigMap API to store attribute-level information or overall information such as configuration or JSON objects. ConfigMap API stores key-value pair configurations that can be used in pods. It is similar to Secret but ConfigMap is more suitable for dealing with non-credential strings
- Uses Secret to configure password, token, and keys without exposing them to images or Pod Specs. Secret can be used as Volume or an environment variable.
...
Includes log and alarm services.

For more information, see [APaas Overview](/docs/apaas/en/latest/about/apaas_overview).
...
For more information, go to EnOS console and select **EnOS API > API Document > Data Service** .
...
- All authorized users can now edit stream analytics tasks.
